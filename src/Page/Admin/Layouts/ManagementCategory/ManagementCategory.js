    ...
    const handleShowDetail = async (madanhmuc) => {
        try {
            ...
            if (error.response?.status === 404) {
                toast.info('Không có sách nào trong danh mục này!', { autoClose: 3000 });
            } else if (error.response?.status === 400) {
                toast.error('Vui lòng cung cấp mã danh mục!', { autoClose: 3000 });
            } else {
        ...
        const fetchCategories = async () => {
            try {
                ...
                if (error.response?.status === 400) {
                    toast.error('Vui lòng nhập tên danh mục để tìm kiếm!', { autoClose: 3000 });
                    setDataCategory([]);
                } else if (error.response?.status === 404) {
                    toast.info('Không tìm thấy danh mục nào phù hợp!', { autoClose: 3000 });
                    setDataCategory([]);
                } else {
                    console.error('Lỗi tìm kiếm:', error);
                    toast.error('Lỗi khi tìm kiếm danh mục!', { autoClose: 3000 });
                    setDataCategory([]);
                }
            }
        };
