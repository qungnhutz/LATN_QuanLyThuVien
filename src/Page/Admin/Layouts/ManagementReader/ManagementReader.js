...
function ManagementReader() {
    ...
    // Fetch tất cả readers khi load trang hoặc sau khi thêm/sửa/xóa
    useEffect(() => {
        ...
        const fetchReaders = async () => {
            try {
                ...
                if (error.response?.status === 400) {
                    toast.error('Thiếu mã sinh viên!', { autoClose: 3000 });
                    setDataReader([]);
                } else if (error.response?.status === 404) {
                    toast.info('Không tìm thấy bạn đọc nào!', { autoClose: 3000 });
                    setDataReader([]);
                } else {
                    console.error('Lỗi tìm kiếm:', error);
                    toast.error('Lỗi khi tìm kiếm bạn đọc!', { autoClose: 3000 });
                    setDataReader([]);
                }
            }
        };
                        ...
                        {currentItems.length > 0 ? (
                            currentItems.map((item) => (
                                <tr key={item._id}>
                                    ...
                                    <td>{item.hoten}</td>
                                    <td>{item.address}</td>
                                    <td>{formatDate(item.brithday)}</td>
                                    <td>{item.sdt}</td>
                                    <td>{item.email}</td>
